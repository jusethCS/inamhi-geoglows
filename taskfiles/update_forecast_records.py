...
def join_forecast_records(comids: list) -> pd.DataFrame:
    """
    ...
    for comid in comids:
        ...
        df = get_geoglows_data(comid=comid, data_type="ForecastRecords")
        df = df.round(3)
...
forecast_records = join_forecast_records(drainge.comid)
forecast_records.to_csv(f"forecast_records/{TODAY}.csv", index=True)
insert_into_db(
    forecast_records, table='forecast_records', con=con, 
    unstack=True, colname="comid")
